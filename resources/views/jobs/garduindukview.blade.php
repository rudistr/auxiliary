...
@section('content')
<div class="container">
<a href="{{ route('jobs.garduinduk')  }}" class="btn btn-success">Create</a>
<div class="row g-3 align-items-center mt-2">
  <div class="col-auto">
 <form action="{{ route('jobs.garduindukview') }}" method="GET">        
  ...
  <button type="submit" class="btn btn-primary col-md-10"><i class="fa fa-print"></i> Filter </button>
</form>
  </div>
</div>
...
<tbody>
    @foreach($datajobs as $index => $job)
     ...
     <td>{{ $job->shift }}</td>
     <td>
       <a href="{{ route('jobs.editgarduinduk',$job->id) }}" class="btn btn-primary btn-sm">Edit</a>       
     </td>                  
    <tr>
    @endforeach
</tbody>
