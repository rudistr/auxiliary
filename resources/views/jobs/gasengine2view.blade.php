...
@section('content')
<div class="container">
<a href="{{ route('jobs.gasengine2')  }}" class="btn btn-success">Create</a>
<div class="row g-3 align-items-center mt-2">
  <div class="col-auto">
 <form action="{{ route('jobs.gasengine2view') }}" method="GET">        
  ...
  <button type="submit" class="btn btn-primary col-md-4"><i class="fa fa-print"></i> Filter </button>
    ...
    <div class="card-header">Gas Engine2</div>       
    <div class="card-body">
    <table class="table table-dark table-striped">        
     <thead>
         <tr>
           ...
           <th>CONS GAS</th>
           <th>LIFE TIME LO</th>
           <th>INTAKE FAN MOTOR CURRENT L1</th>
           <th>INTAKE FAN MOTOR CURRENT L2</th>
           <th>INTAKE FAN MOTOR CURRENT L3</th>
           <th>RADIATOR FAN CURRENT L1 </th>
           <th>RADIATOR FAN CURRENT L2 </th>
           <th>RADIATOR FAN CURRENT L3 </th>
           <th>EXHAUST FAN CURRENT L1</th>
           <th>EXHAUST FAN CURRENT L2</th>
           <th>EXHAUST FAN CURRENT L3</th>
           <th>MAKE UP L.O LEVEL BEFORE</th>           
           <th>MAKE UP L.O LEVEL AFTER</th>
           <th>TOTAL CONS L.O</th>
...
<tbody>
    @foreach($datajobs as $index => $job)
     ...
     <td>{{ $job->surname }}</td>
     <td>{{ $job->shift }}</td>
     <td>{{ $job->group_leader }}</td>     
     <td>{{ $job->supervisor }}</td>
     <td>{{ $job->remarks }}</td>
     <td>
       <a href="{{ route('jobs.editgasengine2',$job->id) }}" class="btn btn-primary btn-sm">Edit</a>       
     </td>                  
    <tr>
    @endforeach
</tbody>
