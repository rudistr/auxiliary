...
@section('content')
<div class="container">
<a href="{{ route('jobs.ac_unit_poly')  }}" class="btn btn-success">Create</a>
<div class="row g-3 align-items-center mt-2">
  <div class="col-auto">
 <form action="{{ route('jobs.ac_unit_polyview') }}" method="GET">        
     ...
     <thead>
         <tr>
          <th>No</th>
           <th>Time</th>
           <th>Machine No.</th>
           <th>CHILLED WTR PRESS IN(3-5 Bar)</th>           
           <th>CHILLED WTR PRESS OUT(3-5 Bar)</th>
           <th>DEW POINT CONTROL TEMP(Max:20C)</th>
           <th>3 WAY VALVE</th>
           <th>ROOM TIC(Max:26C)</th> 
           <th>BAG FILTER</th>
...
<tbody>
    @foreach($datajobs as $index => $job)
     ...
     <td>{{ $job->remarks }}</td>
     <td>
       <a href="{{ route('jobs.editac_unit_poly',$job->id) }}" class="btn btn-primary btn-sm">Edit</a>       
     </td>                  
    <tr>
    @endforeach
</tbody>
