...
@section('content')
<div class="card">
    <div class="card-header">DRYER POLY</div>       
    <div class="card-body">
    <form action="{{ route('jobs.editdryer_poly',$job) }}" method="post">
        @csrf
        @method('put')
        <input type="number" name="id" id="id" class="form-control @error('') is invalid @enderror" value = "{{ $job->id }}"  autofocus>          
