...
@section('content')
<div class="card">
    <div class="card-header">AHU SPINNING 1</div>       
    <div class="card-body">
    <form action="{{ route('jobs.editahu_spinning1',$job) }}" method="post" enctype ="multipart/form-data">  
        @csrf
        @method('put')
        <input type="hidden" name="id" id="id" value = "{{ $job->id }}" class="form-control @error('') is invalid @enderror"   autofocus>          
        ...
        <div class="form-group">
          <label for="mesin">Machine </label>
          <select name="mesin" value = '{{ $job->mesin }}'>
            <option value="1">AC AHU 9003 SPINNING 1</option>
            <option value="2">AC AHU 9103 SPINNING 1</option>
            <option value="3">AC AHU 9203 SPINNING 1</option>
            <option value="4">AC AHU 9303 SPINNING 1</option>
            <option value="5">AC AHU 9403 SPINNING 1</option>
          </select>
        </div>
        @error ('mesin')
           <div class="mt-2 text-danger">{{ $message }}</div>
